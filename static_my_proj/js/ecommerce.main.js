$(document).ready(function(){
    ...
    //   // display new btn ui
    //   var loadtime= 1500
    //   var errorHtml= "<i class='fas fa-warning'></i> An error ocurred"
    //   var errorClasses= "btn btn-danger disabled my-3"
    //   var loadingHtml= "<i class='fas fa-spinner'></i> Loading..."
    //   var errorLoading= "btn btn-success disabled my-3"
    ...
    function stripeTokenHandler(nextUrl, token){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                card.clear()
                if(nextUrl){
                  successMsg = successMsg + "<br><i class='fas fa-spinner'></i> Redirecting..."
                }
                if($.alert){
                  $.alert(successMsg)
                }else{
            ...
            error: function(error){
                console.log(error)
                $.alert({title: "An error ocurred", content:"Please try adding your card again."})
                btnLoad.html(btnLoadDefaultHtml)
                btnLoad.attr('class', btnLoadDefaultClasses)
            }
        })
